/**
 * Deterministic parser for Air Canada (Aeroplan) award search results.
 *
 ...
 *
 * Starter template — refine selectors after inspecting actual Air Canada results DOM.
 */
...
function parseResults(content: string): AwardResult[] {
  ...
  const flightPattern = /(?:AC|Air Canada)\s*(\d{1,4})/gi;
  const milesPattern = /(\d{1,3}(?:,\d{3})*)\s*(?:points|miles)/gi;
  ...
  for (let i = 0; i < count; i++) {
    results.push({
      airline: "Air Canada",
      flightNumber: `AC ${flightNumbers[i]}`,
      departure: times[i * 2] ?? "",
      arrival: times[i * 2 + 1] ?? "",
      origin: "",
      destination: "",
