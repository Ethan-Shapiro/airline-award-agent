/**
 * Deterministic parser for American Airlines award search results.
 *
 ...
 *
 * Starter template — refine selectors after inspecting actual AA results DOM.
 */
...
function parseResults(content: string): AwardResult[] {
  ...
  const flightPattern = /(?:AA|American)\s*(\d{1,4})/gi;
  ...
  for (let i = 0; i < count; i++) {
    results.push({
      ...
      flightNumber: `AA ${flightNumbers[i]}`,
      departure: times[i * 2] ?? "",
      arrival: times[i * 2 + 1] ?? "",
      origin: "",
      destination: "",
